...
# ========== 入口 main.py ==========
FILES["main.py"] = r'''from earth_sim.world import World
...
# ========== earth_sim/__init__.py ==========
FILES["earth_sim/__init__.py"] = ""

# ========== earth_sim/engine/__init__.py ==========
FILES["earth_sim/engine/__init__.py"] = ""

# ========== earth_sim/engine/agents/__init__.py ==========
FILES["earth_sim/engine/agents/__init__.py"] = ""

# ========== earth_sim/engine/config.py ==========
FILES["earth_sim/engine/config.py"] = r'''COUNTRY_COORDS_SIM = {
...
# ========== earth_sim/engine/ecosystem.py ==========
FILES["earth_sim/engine/ecosystem.py"] = r'''import random
...
# ========== earth_sim/engine/politics.py ==========
FILES["earth_sim/engine/politics.py"] = r'''import random
...
# ========== earth_sim/engine/finance.py ==========
FILES["earth_sim/engine/finance.py"] = r'''import random
...
# ========== earth_sim/engine/market.py ==========
FILES["earth_sim/engine/market.py"] = r'''import random
...
# ========== earth_sim/engine/knowledge.py ==========
FILES["earth_sim/engine/knowledge.py"] = r'''from __future__ import annotations
...
# ========== earth_sim/engine/alliance.py ==========
FILES["earth_sim/engine/alliance.py"] = r'''from __future__ import annotations
...
# ========== earth_sim/engine/warfare.py ==========
FILES["earth_sim/engine/warfare.py"] = r'''from __future__ import annotations
...
# ========== earth_sim/engine/agents/internal.py ==========
FILES["earth_sim/engine/agents/internal.py"] = r'''import random
...
# ========== earth_sim/engine/agents/ingo.py ==========
FILES["earth_sim/engine/agents/ingo.py"] = r'''import random
...
# ========== earth_sim/engine/agents/nsag.py ==========
FILES["earth_sim/engine/agents/nsag.py"] = r'''import random
...
# ========== earth_sim/engine/agents/individual.py ==========
FILES["earth_sim/engine/agents/individual.py"] = r'''import numpy as np
import random
...
# ========== earth_sim/ai/__init__.py ==========
FILES["earth_sim/ai/__init__.py"] = ""

# ========== earth_sim/ai/qlearning.py ==========
FILES["earth_sim/ai/qlearning.py"] = r'''import random
...
# ========== earth_sim/utils/__init__.py ==========
FILES["earth_sim/utils/__init__.py"] = ""

# ========== earth_sim/utils/plotter.py ==========
FILES["earth_sim/utils/plotter.py"] = r'''from collections import deque
import matplotlib
...
# ========== earth_sim/world.py ==========
FILES["earth_sim/world.py"] = r'''from __future__ import annotations
import numpy as np
import pandas as pd
import random
import copy
...
from earth_sim.engine.config import COUNTRY_COORDS_SIM, LANGUAGES, RELIGIONS, IDEOLOGIES
...
from earth_sim.engine.alliance import Alliance, AllianceSystem
...
from earth_sim.engine.agents.individual import Individual, Household
...
from earth_sim.engine.finance import CentralBank, StockMarket
from earth_sim.engine.market import ResourceMarket
from earth_sim.engine.knowledge import KnowledgeTree, TechEra, TechTree, TechNode
from earth_sim.engine.alliance import Alliance, AllianceSystem
from earth_sim.engine.warfare import ProxyWarSystem
from earth_sim.engine.agents.internal import InternalAgent, MultinationalCorp
from earth_sim.engine.agents.ingo import INGO
...
class Country:
    def __init__(self, world, name, region, pop, gdp, res, tech, rb, ideology=0, camp=0,
        ...
        self.name = name
        self.region = region
        self.pop_children = pop * 0.35
        self.pop_young = pop * 0.40
        self.pop_middle = pop * 0.20
        self.pop_old = pop * 0.05
        self.material_gdp = gdp * 0.8
        self.service_gdp = gdp * 0.2
        self.resources = res
        self.tech_level = tech
        self.research_budget = rb
        self.stability = 0.7 + random.random() * 0.2
        self.aggression = random.random() * 0.4 + 0.2
        self.ideology = ideology
        self.camp = camp
        self.trade_partners = set()
        self.alive = True
        self.env_pressure = 0.0
        self.education = 0.3
        self.food_reserve = 0.5
        self.war_victory_bonus = 0.0
        self.war_defeat_penalty = 0.0
        self.last_war_year = 0
        self.democracy_movement_cd = 0
        self.government = government
        self.party_support = party_support
        self.election_year = election_year
        self.language = language
        self.religion = religion
        self.main_ideology = main_ideology
        self.cultural_influence = {}
        self.internal_agents: List[InternalAgent] = []
        self.ai_controlled = ai_controlled
        self._prev_gdp = self.gdp
        self._prev_stability = self.stability
        self.founding_population = pop
        self.founding_language = language
        self.founding_ideology = main_ideology
        self.latitude, self.longitude = COUNTRY_COORDS_SIM.get(self.name, (0, 0))
        self.is_federal = False
        self.federal_age = 0
        self.debt = 0.0
    ...
    def population(self, value):
        ...
        if total > 0:
            ratio = value / total
            self.pop_children *= ratio
            self.pop_young *= ratio
            self.pop_middle *= ratio
            self.pop_old *= ratio
    ...
    def cultural_difference(self, other):
        ...
        if self.religion != other.religion:
            diff += 0.3
        if self.main_ideology != other.main_ideology:
            diff += 0.4
        return diff
    ...
    def internal_development(self):
        ...
        if total_pop > carrying_capacity:
            scale = carrying_capacity / total_pop
            self.pop_children *= scale
            self.pop_young *= scale
            self.pop_middle *= scale
            self.pop_old *= scale
        ...
        if per_capita < 20 and self.population < 500:
        ...
        mat_growth *= (0.5 + labor_ratio)
        energy_needed = self.material_gdp * 0.0001
        energy_supplied = (self.resources.get("石油", 0) * 0.3 + self.resources.get("天然气", 0) * 0.2 +
                           self.resources.get("煤炭", 0) * 0.1 + k.energy_tech * 10)
        if energy_supplied < energy_needed * 0.5:
            mat_growth *= energy_supplied / (energy_needed + 1)
        self.material_gdp *= (1 + mat_growth)
        ...
        self.research_budget += 0.008 * (self.gdp / max(1, self.population)) + tech_spillover * 0.01
        if per_capita > 200:
            self.research_budget *= 0.99
        self.research_budget = max(0.1, min(30, self.research_budget))
        self.env_pressure = ((100 - w.global_environment) / 100) * self.education * 0.3
        if self.stability > 0.5:
            k.global_green_policy += self.env_pressure * 0.008
        ...
        if per_capita < 0.3:
            ...
            self.stability -= 0.02
        self.stability += k.sociology * 0.0005
        if self.food_reserve < 0.3:
            self.stability -= 0.03
        scale_penalty = 1.0
        if self.population > 500:
            if self.is_federal:
                scale_penalty += (self.population - 500) / 500 * 1.0
            else:
                scale_penalty += (self.population - 500) / 500 * 2.0
        institutional_entropy = (self.population * self.gdp) / 50000000 * scale_penalty
        self.stability -= institutional_entropy * 0.001
        negentropy = (self.research_budget * 0.063 + k.energy_tech * 0.1) * env_factor
        self.stability += min(negentropy, institutional_entropy * 0.05)
        ...
        serv_share = self.service_gdp / max(1, self.gdp)
        if serv_share > 0.8:
            self.stability -= (serv_share - 0.8) * 0.05
        old_burden = self.pop_old / max(1, self.population)
        if old_burden > 0.2:
            self.research_budget *= 0.999
            self.stability -= old_burden * 0.01
        ...
        for r in list(self.resources.keys()):
            ...
            if self.resources[r] <= 0:
                ...
                w.log(f"{self.name} 的 {r} 枯竭")
                self.material_gdp *= 0.94
        if not self.resources and k.information_tech > 4 and random.random() < 0.15:
            high_res = max(market.prices.items(), key=lambda x: x[1])[0]
            self.resources[high_res] = 20
            w.log(f"{self.name} 因高价刺激开发出 {high_res} 替代技术")
        self.food_reserve += k.agriculture * 0.01 - (100 - w.global_environment) * 0.0003
        self.food_reserve = max(0, min(1, self.food_reserve))
        ...
        if self.alive and self.is_federal and self.federal_age > 10:
            ...
            if random.random() < separation_chance:
                sep_pop = self.population * random.uniform(0.15, 0.3)
                ratio = sep_pop / self.population
                self.pop_children *= (1 - ratio)
                self.pop_young *= (1 - ratio)
                self.pop_middle *= (1 - ratio)
                self.pop_old *= (1 - ratio)
                self.stability += 0.05
                w.log(f"🏛️ {self.name} 的联邦成员和平独立，分离人口 {sep_pop:.0f}M")
                new_name = f"{self.name}自治区"
                w.spawn(new_name, self.region, sep_pop,
        ...
        if not self.alive and k.information_tech > 3 and random.random() < 0.9:
            ...
            self.pop_middle = self.population * 0.20 * 0.85
            self.pop_old = self.population * 0.05 * 0.85
            self.material_gdp *= 0.85
            self.service_gdp *= 0.85
            w.log(f"🔄 {self.name} 重组为流亡政府")
        self._prev_gdp = self.gdp
    ...
    def _update_culture(self, year):
        w = self.world
        for other in w.countries:
            if not other.alive or other.name == self.name:
                continue
            trade_volume = (self.gdp + other.gdp) * 0.01
            alliance_bonus = 1.0
            for al in w.alliances.alliances:
...
class World:
    ...
    def to_dict(self):
        return {
            ...
            "max_gdp": self.max_gdp,
            "war_prob": self.war_prob,
            "proxy_prob": self.proxy_prob,
            "emergence_prob": self.emergence_prob,
            "knowledge": {
                "math": self.knowledge.math,
                "physics": self.knowledge.physics,
                "chemistry": self.knowledge.chemistry,
                "biology": self.knowledge.biology,
                "medicine": self.knowledge.medicine,
                "agriculture": self.knowledge.agriculture,
                "philosophy": self.knowledge.philosophy,
                "sociology": self.knowledge.sociology,
                "energy_tech": self.knowledge.energy_tech,
                "environment_tech": self.knowledge.environment_tech,
                "industry_tech": self.knowledge.industry_tech,
                "information_tech": self.knowledge.information_tech,
                "space_tech": self.knowledge.space_tech,
                "ai_tech": self.knowledge.ai_tech,
                "global_green_policy": self.knowledge.global_green_policy,
                "scientific_knowledge": self.knowledge.scientific_knowledge,
                "quantum_tech": self.knowledge.quantum_tech,
                "unlocked_nodes": [n.name for n in self.knowledge.tech_tree.nodes if n.unlocked]
            ...
            "countries": [{
                ...
                "region": c.region,
                "alive": c.alive,
                "pop_children": c.pop_children,
                "pop_young": c.pop_young,
                "pop_middle": c.pop_middle,
                "pop_old": c.pop_old,
                "material_gdp": c.material_gdp,
                "service_gdp": c.service_gdp,
                "tech_level": c.tech_level,
                "research_budget": c.research_budget,
                "stability": c.stability,
                "aggression": c.aggression,
                "ideology": c.ideology,
                "camp": c.camp,
                "resources": c.resources,
                "war_victory_bonus": c.war_victory_bonus,
                "war_defeat_penalty": c.war_defeat_penalty,
                "last_war_year": c.last_war_year,
                "democracy_movement_cd": c.democracy_movement_cd,
                "education": c.education,
                "food_reserve": c.food_reserve,
                "trade_partners": list(c.trade_partners),
                "government": c.government,
                "party_support": c.party_support,
                "election_year": c.election_year,
                "language": c.language,
                "religion": c.religion,
                "main_ideology": c.main_ideology,
                "ai_controlled": c.ai_controlled,
                "cultural_influence": c.cultural_influence,
                "is_federal": c.is_federal,
                "federal_age": c.federal_age,
                "debt": c.debt,
            ...
            "resource_market": {
                "prices": self.resource_market.prices,
                "trade_agreements": self.resource_market.trade_agreements
            },
            "events_log": self.events_log[-100:],
            "waste_accumulation": self.waste_accumulation,
            "multinationals": [{"name": m.name, "home_country": m.home_country, "power": m.power, "wealth": m.wealth, "branches": m.branches, "alive": m.alive} for m in self.multinationals],
            "ingos": [{"name": i.name, "focus": i.focus, "influence": i.influence, "resources": i.resources, "alive": i.alive} for i in self.ingos],
            "nsags": [{"name": n.name, "type": n.type, "base_country": n.base_country, "strength": n.strength, "alive": n.alive} for n in self.nsags]
        }
    ...
    def log(self, m):
        self.events_log.append(f"[{self.year}] {m}")

    def find(self, n):
        for c in self.countries:
            if c.name == n and c.alive:
                return c
        return None
    ...
    def spawn(self, n, r, pop, gdp, res, tech, rb, ideo, camp, gov="混合", sup=0.5, elec=0, lang="英语", rel="基督教", ide="自由主义"):
        if not self.find(n):
            if elec == 0:
                elec = self.year + random.randint(3, 5)
            c = Country(self, n, r, pop, gdp, res, tech, rb, ideo, camp, gov, sup, elec,
                        language=lang, religion=rel, main_ideology=ide)
            c.spawn_agents()
            c.spawn_households(100)
            self.countries.append(c)
            self.log(f"🆕 {n} 建立（{gov}）")
...
print("项目结构已成功创建！现在可以运行 main.py 开始模拟。")